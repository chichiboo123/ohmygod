  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {playState.isPaused ? (
            ...
            <button
              onClick={pauseGame}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-warning text-white hover:opacity-90 transition-colors"
            >
          ...
          <button
            onClick={resetGame}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-danger text-white hover:opacity-90 transition-colors"
          >
      ...
      {playState.isGameOver && (
        <div className="bg-gradient-to-r from-primary to-purple-500 rounded-xl p-6 text-white text-center space-y-2">
          <span className="material-icons" style={{ fontSize: 48 }}>emoji_events</span>
          <h3 className="text-2xl font-bold">{t('play.gameOver')}</h3>
          {playState.winner && (
            <p className="text-lg">
              {t('play.winner', { name: playState.winner.name })}
              <span className="ml-2">
                {playState.winner.score} {settings.currency}
              </span>
            </p>
          )}
          <button
            onClick={resetGame}
            className="mt-3 px-4 py-2 bg-white text-primary rounded-lg font-medium text-sm hover:bg-white/90"
          >
            {t('play.reset')}
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2">
          <div className={`bg-gradient-to-br ${themeConfig.gradient} rounded-xl border border-border p-4`}>
            {/* Time remaining */}
            {settings.gameOverCondition === 'timeLimit' && (
            ...
            <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 gap-1.5">
              {tiles.map((tile, idx) => {
                const playersHere = playState.players.filter(
                  (p) => p.position === idx && !p.isEliminated
                );
                const isOwned = tile.owner !== null;
                const ownerPlayer = isOwned
                  ? playState.players.find((p) => p.id === tile.owner)
                  : null;

                return (
                  <div
                    key={tile.id}
                    className={`relative rounded-lg p-1.5 text-center min-h-[60px] flex flex-col items-center justify-center border ${
                      tile.isFixed
                  ...
                  >
                    ...
                    {tile.tier1 > 0 && (
                      <span className="text-[8px] text-muted">{tile.tier1}</span>
                    )}
                    {/* Player tokens */}
                    {playersHere.length > 0 && (
                      <div className="flex gap-0.5 mt-0.5">
                        {playersHere.map((p) => (
                          <div
                            key={p.id}
                            className="player-token w-4 h-4 rounded-full border border-white shadow-sm flex items-center justify-center"
                            style={{ backgroundColor: p.color }}
                          >
                            <span className="text-white text-[7px] font-bold">{p.id + 1}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="mt-4 bg-white rounded-xl border border-border p-4">
              <div className="flex items-center justify-center gap-4 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-xl font-medium hover:bg-primary-hover transition-colors disabled:opacity-50"
                >
                  <span className={`material-icons ${diceAnimating ? 'dice-rolling' : ''}`}>
                    casino
                  </span>
                  {t('play.rollDice')}
                </button>
                ...
                {playState.diceResult.length > 0 && (
                  <div className="flex items-center gap-2">
                    {playState.diceResult.map((val, i) => (
                      <div
                        key={i}
                        className="w-10 h-10 rounded-lg bg-slate-900 text-white flex items-center justify-center text-lg font-bold"
                      >
                        {val}
                      </div>
                    ))}
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-2 border border-border rounded-xl text-sm hover:bg-slate-50 transition-colors disabled:opacity-50"
                >
                ...
                {!playState.isAutoMode && (
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="px-2 py-1 border border-border rounded-lg text-xs hover:bg-slate-50"
                    >
              ...
              {showEvent && playState.lastEventCard && (
                <div className="mt-3 flex items-center justify-center">
                  <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border-2 border-primary/30 rounded-xl p-4 text-center max-w-xs animate-bounce">
        ...
        {/* Scoreboard */}
        <div>
          <div className="bg-white rounded-xl border border-border p-4 space-y-3 sticky top-20">
            ...
            {/* Game Info */}
            <div className="pt-2 border-t border-border space-y-1 text-xs text-muted">
